...
class ThetaInfotaxis():
    ...
    def __init__(self, env: GridEnvDeform):
        self.env = env    
        self.O = torch.zeros((len(env.deformations),2916,32))
    ...
    def update_belief(self, belief, pos, observation):
        ...
        for t, theta in enumerate(self.env.deformations):
            P_o_s_theta = torch.all(self.env.get_observation(s = (pos,theta)) == observation) # 0 or 1 
    ...
    def update_belief_superbatched(self, belief,pos):
        """
        Perform a Bayesian belief update in a POMDP with action-dependent transition and observation models.
        Returns the updated belief over states for every action and observation combo (a 3d tensor).
        ...
        # Update Step: Multiply by observation likelihood
        new_belief = torch.einsum('t,to->t', belief, self.O[:,pos,:])
    ...
    def G(self,b,pos):
        ...
        # this is P_o_b_s = P(o|b) = P(o|pos,theta) * b(theta)
        P_o_x_t = torch.zeros((32,len(self.env.deformations)))
        for o, obs in self.env.observations:
            for t, theta in self.env.deformations:
                P_o_x_t[o][t] += torch.all(self.env.get_observation(s = (pos,theta)) == obs) * b[t]
        
        brpime = torch.zeros_like(b)
        for t, theta in self.env.deformations:
            brpime[t] = self.env.get_observation(s = (pos,theta)) * b[t]

        # [H_b_a_o]_i = H(b|a,o_i) = entropy of updated belief shape (numactions, numobservations)
        H_b_a_o = torch.sum(brpime * torch.log2(brpime + (brpime == 0).float()), dim=0)

        G = H_b - torch.einsum('to,t->t', P_o_b_s, H_b_a_o)
...
class IDS():
    ...
    def IDS_action(self, belief,pos):
        """
