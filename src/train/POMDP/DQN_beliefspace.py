...
from environment.env import BeliefSpacePOMDP
import os
import re
...
def train_dqn(args):
    ...
    if run_id is not None:
        ...
        run = wandb.init(
            project="DQN - MDP - ObservableDeformedGridworld",
            config=config,
            sync_tensorboard=True,  # auto-upload sb3's tensorboard metrics
            monitor_gym=True,  # auto-upload the videos of agents playing the game
            save_code=True,  # optional
    ...
    if args.run_id is not None:
        ...
        model = DQN("MultiInputPolicy",env,batch_size=batch_size,gamma=gamma, 
                ...
                train_freq=(2,"step"), gradient_steps=1, exploration_fraction=0.1, exploration_final_eps=0.05)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total_timesteps", type=int, default=20000000) # env steps
    parser.add_argument("--target_update", type=int, default=10000) # in env steps
