...
from sofa_env.sofa_templates.camera import PhysicalCamera, Camera, CAMERA_PLUGIN_LIST
...
from sofa_env.scenes.grasp_lift_touch.sofa_objects.point_of_interest import ControllablePointOfInterest, PointOfInterest, POI_PLUGIN_LIST
...
def createScene(
    ...
    # random rotation with bounds
    n = 10
    random_rotation = np.random.uniform(-360/n, 360/n, size=3)
    ...
    # # Add to createScene() after collision models
    # scene_node.addObject('DefaultContactManager', response="FrictionContact")
    # scene_node.addObject('LocalMinDistance', 
    #     alarmDistance=10, 
    #     contactDistance=5,  # Increased for organ-scale collisions
    #     angleCone=0.2
    # )

    # In createScene function after creating liver and gallbladder:

    # Liver force field
    # liver_force_field = liver.node.addObject('LinearForceField', name='LiverForceField', force=[5000.0, 0.0, 0.0], arrowSizeCoef=10.0)

    # # Gallbladder force field
    ...
    gallbladder_force_field.showArrowSize =.010  # Visualize forces
    ...
    return {
        ...
        "contact_listener": {
            ...
            "cauter_liver": cauter_liver_listener,
            "gripper_liver": gripper_liver_listener,
            # "liver_gallbladder": liver_gallbladder_listener,
        },
        "poi": poi,
        "camera": camera,
        "liver": liver,
